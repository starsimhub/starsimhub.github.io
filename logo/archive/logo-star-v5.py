...
if plot1:
    ...
    inds, csf_x, csf_y = make_splines(arms, 1.0, 0.4, ninterp, rf=rf, off=0*np.pi/arms)
    fig1 = plt.figure(figsize=(6, 6))
    plt.fill(csf_x, csf_y, "k")
    for z in np.linspace(0,0.8,20):
        inds, csf_x, csf_y = make_splines(arms, 1.0*z, 0.4*z, ninterp, rf=rf*z, off=5*z*np.pi/arms)
        plt.fill(csf_x, csf_y, "gold", alpha=0.15)
    plt.axis("equal")
    plt.axis("off")
    plt.show()